...
import { X, Sheet, FileCode, Check, RefreshCw, AlertCircle, Download, Upload, RotateCcw } from 'lucide-react';
...
export const SyncSheetModal: React.FC<SyncSheetModalProps> = ({
  ...
  const handleSync = async () => {
    ...
    if (success) {
      setSyncStatus({
        type: 'success',
        message: '구글 스프레드시트의 최신 순서와 곡 목록이 성공적으로 동기화되었습니다!'
      });
    } else {
      setSyncStatus({
        type: 'error',
        message: '동기화 실패: URL 또는 웹 앱 배포 권한([모든 사용자])을 확인해주세요.'
      });
    }
  };
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 text-xs sm:text-[13px]">
          ...
          <div className="p-4 rounded bg-[#fdfaf1] border border-[#3d2b1f]/15 space-y-3">
            ...
            <p className="text-[11.5px] text-[#5d4037] leading-relaxed font-batang">
              배포한 Google Apps Script 웹 앱 URL을 입력하면 스프레드시트의 최신 행사 순서 및 찬양 목록이 즉시 반영됩니다.
            </p>
            ...
            <div className="space-y-2">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-[#fdfaf1] border border-[#3d2b1f]/30 rounded text-xs text-[#2a1b0a] focus:outline-none focus:border-[#3d2b1f]"
                />
                <button
                  onClick={handleSync}
                  disabled={isSyncing}
                  className="px-3.5 py-2 bg-[#3d2b1f] hover:bg-black disabled:opacity-50 text-[#fdfaf1] font-sans font-bold rounded text-xs flex items-center space-x-1.5 transition-all cursor-pointer shadow-xs"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${isSyncing ? 'animate-spin' : ''}`} />
                  <span>{isSyncing ? '동기화 중...' : '불러오기'}</span>
                </button>
              </div>
